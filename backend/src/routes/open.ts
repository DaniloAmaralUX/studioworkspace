...
export const openRoutes: FastifyPluginAsyncZod = async (app) => {
  ...
  app.post(
    ...
    { schema: { params: idParams, body: openSchema } },
    async (req, reply) => {
      ...
      try {
        await openTarget(targetPath, req.body.with)
        return { ok: true, opened: targetPath, with: req.body.with, stamped }
      } catch (err) {
        return reply
          .code(501)
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const updated = await setCloneDir(req.params.id, dest)
      reply.code(201)
      return updated
    },
  )
